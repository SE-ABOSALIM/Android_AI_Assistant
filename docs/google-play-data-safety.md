...
This draft describes the current repository, including command-history privacy
hardening completed after Production Hardening 5/11.
...
| --- | --- | --- |
| Does the app collect any required user-data type? | **Yes** | Recognized command content, launcher-visible installed apps, an app-scoped Android ID, language, custom commands, command results, and diagnostics leave the device. The Android-configured speech-recognition service may also receive microphone audio and, depending on the device, provider, and settings, may process it locally or remotely. |
| Does the app share user data? | **No, based on current source** | The Android client sends data only to the first-party backend. No ads, analytics, crash-reporting, remote-monitoring, or third-party AI SDK/API is integrated. The App uses whichever speech-recognition service Android has configured and does not select one universal provider. The provider/service-provider classification can therefore vary by device and should not be represented as a fixed integration. |
...
| Contacts — Contacts | **Yes, conservatively** | No | No if the user saves the same data type in an optional custom-command step; ordinary command processing is ephemeral | Optional | App functionality. A contact name or number spoken/typed in a call command is transmitted as command content but is not retained in command history. The App does **not** upload address-book query results or the locally resolved number. |
| App activity — App interactions | **Yes** | No | No | Required | App functionality. Intent, accepted/failed status, bounded error code, confidence, and processing-time metadata are stored. Raw command text and prediction parameters are not stored in command history. |
| App activity — In-app search history | **Yes** | No | No if the user saves a query in an optional custom-command step; ordinary `/predict` search processing is ephemeral | Optional | App functionality. Search text is transmitted when the user searches, processed for the request, and not retained in command history. |
...
| App activity — Other user-generated content | **Yes** | No | No because optional custom-command names and steps persist; ordinary voice/manual command text is processed ephemerally | Required | App functionality. Voice/manual command text is required for the assistant and is transmitted but not retained in command history. Optional custom-command names and steps share this type and are stored when the user creates them. Because the same type is required for primary functionality, mark the type required. |
...
records have no inactivity expiry. Raw `/predict` command content is processed
in memory and discarded after request processing. Raw command text is not persisted
and raw prediction parameters are not persisted in command history.
“No app delete” does not rule out a manual administrator/database operation.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1. Recognized voice text | `SpeechRecognizer.RESULTS_RECOGNITION`; `RECORD_AUDIO` | Yes | `POST /predict`: `text` | Processed in backend memory; raw text is not persisted in command history | Not in production Android/backend logs | Resolve and execute commands | Ephemeral request processing; no command-history content copy to delete | No first-party backend sharing; recognizer caveat applies | Other user-generated content; also Contacts/Search where applicable |
...
| 3. Raw microphone audio | Android `SpeechRecognizer`; `RECORD_AUDIO`; audio callback ignored | Recognizer/service accesses it; App does not consume buffer | No Android AI Assistant backend endpoint/field. The Android-configured recognition service may process it remotely | None in App/backend; handling outside the App varies with the recognizer configured on the device | No | Provide voice-command recognition through Android's configured service | Controlled by the Android-configured recognizer; no universal provider is assumed | No under the current user-initiated/system-service analysis; reassess only if the App directly integrates or selects a provider | Voice recordings: Yes conservatively for the global form; not received by App backend |
| 4. Contacts/names/numbers | `ContactsContract.CommonDataKinds.Phone`; `READ_CONTACTS` | Names/numbers queried and matched locally | Local query results: none. A name/number included in command text goes to `/predict` | Local result: none. Command text and derived `contact_name` are not retained in command history; a user-created custom-command step remains a separate optional persistent path | No content logging | Resolve a requested call target | Ordinary command content is ephemeral; explicitly saved custom-command data persists until that command is deleted | No | Contacts: Yes conservatively for transmitted command contact content |
...
| 6. Camera/torch/media | `CameraManager`, system camera intent; `CAMERA` for torch | Torch state and camera UI automation | Only command text/derived camera-facing parameter via `/predict`; no media | Command text and derived parameter are not retained in command history; no photo/video store | No media logging | Torch control and user-requested system camera action | Media remains under system camera; ordinary command content is ephemeral | System camera is user-requested platform app | Photos/videos: No |
...
| 13. Session ID | Random UUID held by `AssistantSession` | Yes | App-catalog paths/body and `/predict` | Used in request/in-memory session paths; not retained in command history. App catalog persists by authenticated device instead | Not logged | Correlate local assistant session/request | Memory clears when session ends; no command-history copy | No | Device or other IDs |
...
| 18. Command history | Backend prediction result | No | Created from `/predict` | `command_history`: internal device FK, intent, language, accepted flag, confidence, bounded error code, processing time, and timestamp only | Safe status/timing remains | Persist minimal operational outcomes without a transcript | Repository delete methods exist but no exposed app/API route; metadata is indefinite | No | App interactions and Diagnostics; no UGC/Search/Contacts content in history |
| 19. Intent/parameters/confidence | Rule/ML/validation output | Returned to Android | `/predict` response | Intent and confidence persist as bounded metadata; raw parameters remain in request/response memory and are not retained in command history; raw label/top predictions are not retained | Raw parameters removed from logs | Command execution/validation | Parameters are ephemeral for ordinary predictions; intent/confidence metadata is indefinite | No | App interactions; transmitted parameter content can map to UGC/Search/Contacts but is not retained by history |
...
- Migration 009 permanently drops raw text and raw parameter columns from
  command history, removing existing values without an archive or replacement.
...
Android `SpeechRecognizer` is a platform API. The App always creates Android's
generally configured recognizer instead of explicitly selecting an on-device
recognizer. The configured service can vary by device, provider, settings,
language, or model configuration and may process microphone audio on the device
or remotely. The App does not receive audio bytes or send raw microphone audio
...
| --- | --- | --- |
| Recognized text goes to `/predict`, is processed in memory, and is not retained in command history | Disclosed as transmitted but not retained in history | Other user-generated content: Yes because it leaves the device; ordinary command processing is ephemeral, while optional custom commands remain persistent |
| Raw audio has no App/backend field or write path, but the Android-configured recognition service may process it remotely | Backend receives recognized text, not raw audio; device-, provider-, and settings-dependent local or remote processing is disclosed | Voice recordings: Yes conservatively because remote processing remains possible; no universal provider or provider-verification blocker assumed |
...
| Contact records resolve locally; spoken contact content is transmitted but not retained in command history | Both paths are explicitly distinguished | Contacts: conservative Yes for transmitted command content; no upload of address book results |
...
- Durable schema: migrations 001–009 and the three database repositories
