...
from V3.app_catalog.constants import MAX_INDEXED_CANDIDATE_APPS, MIN_NGRAM_MATCH_RATIO
from V3.app_catalog.models import AppCatalogEntryRecord
from V3.app_catalog.text_normalization import _fast_tokens, _ngrams, _normalize_words
...
def _build_catalog_search_index(entries: List[AppCatalogEntryRecord]) -> Dict[str, Dict[str, Set[int]]]:
    ...
    for app_index, app in enumerate(entries):
        should_index_compound_aliases = _has_compound_display_alias(app)
        for alias in app.match_aliases:
            ...
            tokens = _fast_tokens(normalized)
            _add_index_value(exact_index, normalized, app_index)

            if should_index_compound_aliases:
...
def _has_compound_display_alias(app: AppCatalogEntryRecord) -> bool:
    return any(len(_fast_tokens(_normalize_words(alias))) >= 2 for alias in [app.label, *app.aliases])
